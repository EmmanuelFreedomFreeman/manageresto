...
function Orders() {
    ...
    const toprint = () => {
        ...
        <div>
            <div style='text-align:center'>
                ...
                ${display?.data?.date ? (
                    `<p>DATE : ${new Date(display?.data?.date).getDate()} / ${new Date(display?.data?.date).getMonth()}  / ${new Date(display?.data?.date).getFullYear()}  : ${new Date(display?.data?.date).getHours()}H - ${new Date(display?.data?.date).getMinutes()}Min - ${new Date(display?.data?.date).getSeconds()}Sec - ${new Date(display?.data?.date).getMilliseconds()}Milli </p>`
    ...
    const displayorders = (index: number) => {
        const temp = {data : order[index],ind : index}
        setDisplay(temp) 
  ...
  return (
    <div className='flex justify-between p-3'>
        ...
        <div className=''>
            <h1 className='text-center text-2xl font-bold underline'>NEW ORDERS</h1>
            <div className='overflow-y-scroll max-h-96 p-4'>
            {order.map((value:any,index:number) => (
                <div className={value?.etat =='new order'?'border-2 mt-5 border-gray-500 p-4 ':'hidden'} key={index}>
